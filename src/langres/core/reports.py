...
class BlockerEvaluationReport(BaseModel):
    ...
    def diagnose(
        ...
        n_missed: int = 20,
        n_false_positives: int = 20,
        min_fp_score: float = 0.7,
    ) -> "DiagnosticExamples":
        """Extract diagnostic examples for manual inspection.

        Similar to scikit-learn's pattern of showing misclassified samples,
        this provides concrete failure examples to understand where the
        blocker succeeded and failed.

        Args:
            candidates: Candidate pairs generated by blocker
            entities: Dict mapping entity ID to entity data (must have 'name' or similar)
            n_missed: Maximum number of missed matches to return
            n_false_positives: Maximum number of false positives to return
            min_fp_score: Minimum score threshold for "high-scoring" false positives

        Returns:
            DiagnosticExamples with concrete failure cases

        Raises:
            ValueError: If report was not created with gold cluster information
        ...
        Note:
            This requires access to the original candidates and entity data.
        ...
        if not hasattr(self, "_gold_clusters"):
            raise ValueError(
                "Cannot diagnose without gold clusters. "
                "This report was not created with gold cluster information. "
                "Use evaluate_blocker_detailed() to create reports with diagnosis support."
            )

        gold_clusters = self._gold_clusters

        # Extract examples
        missed = extract_missed_matches(candidates, gold_clusters, entities, n=n_missed)
        false_pos = extract_false_positives(
            candidates, gold_clusters, entities, n=n_false_positives, min_score=min_fp_score
        )
