...
class Blocker(ABC, Generic[SchemaT]):
    ...
    def evaluate(
        ...
        """Evaluate blocking quality with ground truth labels.

        Convenience method that delegates to metrics.evaluate_blocking() for
        actual computation. Measures how well this blocker captures true
        duplicate pairs in the candidate generation stage.
        ...
        Returns:
            CandidateStats with blocking performance metrics:
        ...
        Example:
            ...
            >>> stats = blocker.evaluate(candidates, gold_clusters)
            >>> print(f"Blocking recall: {stats.candidate_recall:.2%}")
            >>> if stats.candidate_recall < 0.90:
            ...     print("Warning: Low recall! Increase k_neighbors")
        ...
        Note:
            ...
            metrics.evaluate_blocking(candidates, gold_clusters) directly.
        """
        from langres.core.metrics import evaluate_blocking

        return evaluate_blocking(candidates, gold_clusters)
